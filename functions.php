	...
	if ($atts['layout'] === 'team') {
		...
		if ($query->have_posts()) {
			while ($query->have_posts()) {
				...
				if ($person_image) {
					echo '<img src="' . esc_url($person_image['url']) . '" alt="' . esc_attr($person_image['alt']) . '" class="img-thumbnail rounded-circle border-0" />';
				}
				echo '</div>';
				...
				echo '<div class="card card-body p-2 pt-3 border-0">' . $person_bio . '</div>';  // Remove esc_html
